...
const SouthIndianChart: React.FC<SouthIndianChartProps> = ({ data }) => {
    ...
    if (isCenter) {
      if (gridIdx === 5) return (
        <div key={gridIdx} className="col-span-2 row-span-2 flex items-center justify-center p-2 sm:p-4 border border-[#431407]/20">
          <svg viewBox="0 0 100 100" className="w-16 h-16 sm:w-24 md:w-32 fill-[#7C2D12] opacity-40">
    ...
    return (
      ...
      >
        <div className="flex flex-wrap gap-0.5 sm:gap-1 justify-center items-center h-full content-center">
          {planets.map((p, i) => (
            <div key={i} className="text-[10px] sm:text-xs md:text-sm font-black text-[#431407]">
  ...
  return (
    <div className="w-full max-w-[320px] sm:max-w-md mx-auto bg-white/20 border-2 border-[#431407]/60 shadow-xl grid grid-cols-4 grid-rows-4 rounded-xl overflow-hidden backdrop-blur-sm">
